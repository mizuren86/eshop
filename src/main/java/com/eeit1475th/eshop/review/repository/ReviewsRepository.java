...
//    Page<Reviews> findByProductsOrderByUpdatedAtDesc(Products product, Pageable pageable); 
	...
	// 計算平均評分
	@Query("SELECT AVG(r.rating) " + "FROM Reviews r " + "JOIN r.products p " + "WHERE p.productId = :productId")
	...
	@Query("SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto(r.users.username, r.rating, r.comment, r.photo, r.updatedAt) "
			+ "FROM Reviews r " + "JOIN r.products p " + "JOIN r.users u " + "WHERE p.productId = :productId "
			+ "ORDER BY r.updatedAt DESC")
	Page<ReviewsDto> findByProductsOrderByUpdatedAtDesc(@Param("productId") Integer productId, Pageable pageable);

	// 按評分篩選 (使用相同的JOIN和DTO結構)
	@Query("SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto(r.users.username, r.rating, r.comment, r.photo, r.updatedAt) "
			+ "FROM Reviews r " + "JOIN r.products p " + "JOIN r.users u " + "WHERE p.productId = :productId "
			+ "AND (:rating IS NULL OR r.rating = :rating) " + "ORDER BY r.updatedAt DESC")
	Page<ReviewsDto> findByProductIdAndRating(@Param("productId") Integer productId, @Param("rating") Integer rating,
			Pageable pageable);

	@Query("SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto2("
			+ "r.reviewId, p.productId, p.productName, r.rating, r.comment, r.photo, r.updatedAt) " + "FROM Reviews r "
			+ "JOIN r.products p " + "JOIN r.users u " + "WHERE u.userId = :userId " + "ORDER BY r.updatedAt DESC")
	Page<ReviewsDto2> findReviewsByUserIdWithProductInfo(@Param("userId") Integer userId, Pageable pageable);

	@Query("SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto2("
			+ "r.reviewId, p.productId, p.productName, r.rating, r.comment, r.photo, r.updatedAt) " + "FROM Reviews r "
			+ "JOIN r.products p " + "JOIN r.users u " + "WHERE u.userId = :userId "
			+ "AND p.productName LIKE %:productName% " + // 這裡加入了模糊搜尋
			"ORDER BY r.updatedAt DESC")
	Page<ReviewsDto2> findReviewsByUserIdWithProductInfoAndProductName(@Param("userId") Integer userId,
			@Param("productName") String productName, // 新增了 productName 參數
			Pageable pageable);

	@Query(value = "SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto3("
		    ...
		    + "JOIN r.products p "
		    + "WHERE r.comment LIKE %:comment%")
		Page<ReviewsDto3> findAllReviewsWithUserAndProductInfoWithCommentLike(
		    @Param("comment") String comment,
		    Pageable pageable);

	@Query("SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto3("
			+ "u.username, p.productName, r.reviewId, u.userId, p.productId, "
			+ "r.rating, r.comment, r.photo, r.updatedAt) " + "FROM Reviews r " + "JOIN r.users u "
			+ "JOIN r.products p " + "ORDER BY r.updatedAt DESC")
	Page<ReviewsDto3> findAllReviewsWithUserAndProductInfo(Pageable pageable);
	...
	@Query(value = "SELECT new com.eeit1475th.eshop.review.dto.ReviewsDto3("
		    ...
		    + "JOIN r.products p "
		    + "WHERE (:rating IS NULL OR r.rating = :rating) ")
		Page<ReviewsDto3> findAllReviewsWithUserAndProductInfoByRating(
