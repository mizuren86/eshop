...
public class EcpayFunction {
	...
	private static final Logger logger = LogManager.getLogger(EcpayFunction.class);
...
//	}
	public final static String genCheckMacValue(String key, String iv, Object obj) {
		...
		try {
			// 依照欄位名稱排序後，組合成 &參數字串（排除 CheckMacValue 與 PaymentToken）
			for (String name : fieldNames) {
				if (!name.equals("CheckMacValue") && !name.equals("PaymentToken")) {
					Method method = cls.getMethod("get" + name);
					Object value = method.invoke(obj);
					if (value != null) {
						data.append("&").append(name).append("=").append(value.toString());
					}
				}
			}
			// 組合 HashKey、參數字串與 HashIV
			String raw = "HashKey=" + key + data.toString() + "&HashIV=" + iv;
			logger.debug("Raw string: {}", raw);

			// URL encode 後轉成小寫，再依 .NET 轉換規則處理
			String encoded = urlEncode(raw).toLowerCase();
			logger.debug("Encoded string: {}", encoded);
			String netEncoded = netUrlEncode(encoded);
			logger.debug("NetEncoded string: {}", netEncoded);

			// 計算 SHA-256 hash 並轉為大寫 HEX 字串返回
			String computedHash = hash(netEncoded.getBytes("UTF-8"), "SHA-256").toUpperCase();
			logger.debug("Computed CheckMacValue: {}", computedHash);
			return computedHash;
		} catch (Exception e) {
			throw new EcpayException(ErrorMessage.GEN_CHECK_MAC_VALUE_FAIL);
		}
...
//		return url.replaceAll("-", "%2d").replaceAll("!", "%21").replaceAll("_", "%5f").replaceAll("~", "%7e")
//				.replaceAll("*", "%2a").replaceAll("(", "%28").replaceAll(")", "%29").replaceAll(".", "%2e")
//				.replaceAll("+", "%20").replaceAll(":", "%3a").replaceAll("/", "%2f").replaceAll("=", "%3d");
//	}

//	private static String netUrlEncode(String url) {
//		String netUrlEncode = url.replaceAll("%21", "\\!")
//								 .replaceAll("%28", "\\(")
//								 .replaceAll("%29", "\\)")
//								 .replaceAll("%2d", "\\-")
//								 .replaceAll("%5f", "\\_")
...
//								 .replaceAll("%2e", "\\.")
//								 .replaceAll("%20", "\\+")
//								 // 新增強制轉換：先轉換原始字元，再把已轉換的對照回來（注意順序避免雙重替換）
//								 .replaceAll("\\:", "%3a")
//								 .replaceAll("\\/", "%2f")
//								 .replaceAll("\\-", "%2d");
//		return netUrlEncode;
