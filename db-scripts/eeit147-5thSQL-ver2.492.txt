...
CREATE TABLE selected_store(
    selected_store_id INT PRIMARY KEY IDENTITY(1, 1),
    CVS_store_id NVARCHAR(255),
    CVS_store_name NVARCHAR(255),
    CVS_address NVARCHAR(255),
    CVS_telephone NVARCHAR(255)
);
...
CREATE TABLE orders (
    ...
    merchant_trade_no NVARCHAR(20) NULL,  -- 新增：綠界金流所需的廠商交易編號
    order_date DATETIME DEFAULT GETDATE(),
    payment_method NVARCHAR(50),
    payment_status VARCHAR(50) CHECK (payment_status IN ('待處理', '已付款', '失敗', '已退款')),
    shipping_method VARCHAR(50),  -- 新增：送貨方式，宅配、新竹貨運、外島郵寄、7-11 取貨
    shipping_address VARCHAR(255),
    recipient_name VARCHAR(100),  -- 新增：收件人姓名
    recipient_phone VARCHAR(20),  -- 新增：收件人電話
    order_remark TEXT,  -- 新增：訂單備註（記錄客戶留言或特殊要求）
    selected_store_id INT NULL,  -- 新增：選擇取貨商店
    shipping_fee DECIMAL(10,2),  -- 新增：運費
    coupon_discount DECIMAL(10,2),  -- 新增：優惠金額
    shipping_status VARCHAR(50) CHECK (shipping_status IN ('處理中', '已出貨', '已送達', '已取消')),
    FOREIGN KEY (user_id) REFERENCES users(user_id),
    FOREIGN KEY (selected_store_id) REFERENCES selected_store(selected_store_id)
);
...
CREATE TABLE shipment(
    ...
    delivery_status VARCHAR(50) CHECK (delivery_status IN ('待處理', '運送中', '已送達')),
    FOREIGN KEY (order_id) REFERENCES orders(order_id)
); 
